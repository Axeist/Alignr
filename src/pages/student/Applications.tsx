            ...
            <Card className="glass cursor-move hover:shadow-lg transition-shadow">
              <CardContent className="pt-4">
                <div className="space-y-2">
                  ...
                  {(app.status === "interview_scheduled" || app.status === "interview") && app.interview && (
                    <div className="mt-3 pt-2 border-t border-gray-200 space-y-2">
                      {app.interview.interview_date && (
                        <div className="text-xs text-gray-300">
                          <Calendar className="h-3 w-3 inline mr-1" />
                          <strong>Date:</strong> {new Date(app.interview.interview_date).toLocaleDateString('en-US', {
                            weekday: 'short',
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric'
                      ...
                      {app.interview.interview_time && (
                        <div className="text-xs text-gray-300">
                          <Clock className="h-3 w-3 inline mr-1" />
                          <strong>Time:</strong> {new Date(`2000-01-01T${app.interview.interview_time}`).toLocaleTimeString('en-US', {
                            hour: 'numeric',
                            minute: '2-digit',
                            hour12: true
                          })}
                      ...
                      {onRescheduleInterview && app.interview?.reschedule_status !== "pending" && (
                        <Button
                          size="sm"
                          variant="outline"
                          className="w-full border-primary text-primary hover:bg-primary/10 text-xs px-2 py-1.5 h-auto min-h-[32px] flex items-center justify-center gap-1.5 mt-2"
...
export default function Applications() {
    ...
    mutationFn: async () => {
      ...
      const { error: interviewError } = await supabase
        ...
        .insert({
          ...
          mode: interviewMode,
          status: "pending",
          meeting_link: null, // Meeting link will be generated by alumni when they proceed
  ...
  const handleSubmitSchedule = () => {
    ...
    // Meeting link is auto-generated, so we don't require it from students
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {(app.status === "interview_scheduled" || app.status === "interview") && app.interview && (
                          <div className="flex flex-col gap-2">
                            ...
                            {app.interview?.reschedule_status !== "pending" && (
                              <Button
                                size="sm"
                                variant="outline"
                                className="border-primary text-primary hover:bg-primary/10"
        ...
        <Dialog open={scheduleDialogOpen} onOpenChange={setScheduleDialogOpen}>
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              ...
              {/* Meeting link is auto-generated - students don't need to provide it */}
              {!isEditing && (interviewMode === "online" || interviewMode === "video_call") && (
                <div className="space-y-2">
                  <div className="text-sm text-gray-400 italic">
                    A meeting link will be automatically generated when the interview is scheduled.
                  </div>
                </div>
              )}
              {isEditing && (interviewMode === "online" || interviewMode === "video_call") && (
